...
//        $emptyConf->emptyConf($conference,$em);
    ...
    $defaultCategory =    $this->getDoctrine()
      ...
      ->findOneBy(array('conference' => $conference->getId()));
    ...
    $role = 'Chair';
    if(null == $chairRoleType = $this->getDoctrine()
        ->getRepository('fibeWWWConfBundle:RoleType')
        ->findOneBy(array('name' => $role, 'conference' => $conference->getId()))
    ){
      $chairRoleType = new RoleType();
      $chairRoleType->setConference($conference)
        ->setName($role)
        ->setLabel($role);
    ...
    $role = 'Presenter';
    if(null == $presenterRoleType = $this->getDoctrine()
        ->getRepository('fibeWWWConfBundle:RoleType')
        ->findOneBy(array('name' => $role, 'conference' => $conference->getId()))
    ){
      $presenterRoleType = new RoleType();
      $presenterRoleType->setConference($conference)
        ->setName($role)
        ->setLabel($role);
      $em->persist($presenterRoleType);
    }
