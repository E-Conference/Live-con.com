...
class FOSUBUserProvider extends BaseClass
{
    ...
    { 
        ...
        {   //social service user not found
            ...
            {   //no user with this social service Id and not logged
                // => creating a new user with login = password = email   
    ...
    //just login 
    private function login($user,$serviceName,$response)
    {
    ...
    private function firstEnrich($user,$serviceName,$response,$socialServiceId)
    { 
    ...
    //enrich account on demand
    private function enrich($user,$serviceName,$response)
    { 
        $setter = 'set' . ucfirst($serviceName) . 'AccessToken';
        $user->$setter($response->getAccessToken());
    ...
    private function create($serviceName,$response,$socialServiceId)
    {
            ...
            $this->session->getFlashBag()->add('success', 'An account have been created with login = '.$response->getEmail().'. '
